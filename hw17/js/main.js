...
class User {
  ...
  render() {
    return `
        ...
        <div class="user__info--data">
              <img src="images/users/${this.img}.png" alt="Jack Smith" height="80px">
            ...
            <div class="user__info--role student">
              <img src="images/roles/${this.role}.png" alt="student" height="30px">
              <p><b>${this.role}</b></p>
            </div>
        </div>
  ...
  renderCourses() {
    return this.courses.length > 0 ? this.courses.map(({ title, mark }) => `
        <p class="user__courses--course ${this.role}">${title} <span class="${this.renderMark(mark)}"><b>${this.renderMark(mark)}</b></span></p>
    `).join('') : '';
  }
